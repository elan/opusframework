...
curl -0 -L http://downloads.xiph.org/releases/opus/opus-1.2.1.tar.gz | tar zx --strip 1 -C libopus-src/
curl -0 -L http://downloads.xiph.org/releases/opus/opusfile-0.10.tar.gz | tar zx --strip 1 -C libopusfile-src/
curl -0 -L http://downloads.xiph.org/releases/ogg/libogg-1.3.3.tar.gz | tar zx --strip 1 -C libogg-src/
